...
#    → démarre Gemma4 + InfluxDB (Docker) + Edge Node (Python)
...
.PHONY: all help start start-codespaces start-rpi start-local stop restart status logs clean \
        ...
        update-passwords \
        api-status api-login api-data api-alerts api-diagnostic \
...
## start-codespaces : Codespaces — données réelles Arduino via bridge PC + Gemma4 Docker
start-codespaces: .env
	@echo ""
	@echo -e "$(BOLD)$(CYAN)╔══════════════════════════════════════════════════════════╗$(NC)"
	@echo -e "$(BOLD)$(CYAN)║   GNL IoT Edge — GitHub Codespaces (Données Réelles)     ║$(NC)"
	@echo -e "$(BOLD)$(CYAN)║   Arduino → Bridge PC → MQTT Public → Edge Node → Gemma4 ║$(NC)"
	@echo -e "$(BOLD)$(CYAN)╚══════════════════════════════════════════════════════════╝$(NC)"
	@echo ""
	...
	@echo ""
	@echo -e "$(GREEN)► Étape 2/4 — Démarrage InfluxDB (Docker)...$(NC)"
	@if docker info > /dev/null 2>&1; then \
	  $(COMPOSE) up -d influxdb 2>/dev/null \
	    && echo -e "$(GREEN)  ✓ InfluxDB démarré$(NC)" \
	...
	@echo ""
	@echo -e "$(GREEN)► Étape 4/4 — Démarrage Edge Node (en attente données Arduino)...$(NC)"
	@echo ""
	@echo -e "$(BOLD)━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━$(NC)"
...
start-local: check-deps .env
	...
	@$(MAKE) build --no-print-directory
	@echo -e "$(GREEN)► Lancement services (Gemma4 + InfluxDB + Grafana)...$(NC)"
	@$(COMPOSE) --profile ai up -d mosquitto influxdb grafana gnl_edge mongodb
	@sleep 30
	@$(MAKE) status --no-print-directory
...
status:
	...
	@echo -n "  Gemma4       : " && \
	 ...
	 && echo -e "$(GREEN)● UP$(NC)" || echo -e "$(YELLOW)● DOWN (démarrer avec make start-gemma4)$(NC)"
	@echo -n "  InfluxDB     : " && \
...
ngrok-start:
	...
	curl -sf http://localhost:4040/api/tunnels > /dev/null 2>&1 \
	  ...
	  || (echo -e "$(RED)  ✗ Tunnel non démarré$(NC)" && cat /tmp/ngrok.log | tail -10)

## ngrok-stop       : Arrête ngrok
ngrok-stop:
	@pkill -f "ngrok http" 2>/dev/null && echo -e "$(GREEN)✓ ngrok arrêté$(NC)" || true
...
## mqtt-listen      : Écoute tous les topics (broker public HiveMQ)
mqtt-listen:
...
## mqtt-publish-test : Publie un message de test
mqtt-publish-test:
...
## setup-influx     : Configure InfluxDB
setup-influx:
	@sleep 3
...
## influx-backup    : Sauvegarde InfluxDB
influx-backup:
...
## api-login        : Obtient un token JWT
api-login:
...
## api-diagnostic   : Diagnostic Gemma4
api-diagnostic:
	@TOKEN=$$(curl -s -X POST http://localhost:5000/api/v1/auth/login \
	  -H "Content-Type: application/json" \
	  -d '{"username":"admin","password":"admin_GNL_2025!"}' \
...
## test-coverage    : Tests avec couverture HTML
test-coverage:
	@$(PIP) install --break-system-packages -q pytest pytest-cov 2>/dev/null || true
...
## check-ports      : Ports requis
check-ports:
	@for port in 5000 8080 8086 3000; do \
	  echo -n "  Port $$port : "; \
	  ss -tlnp 2>/dev/null | grep -q ":$$port " \
	    && echo -e "$(GREEN)● occupé$(NC)" || echo -e "$(YELLOW)○ libre$(NC)"; \
	done

## check-serial     : Ports série Arduino
check-serial:
...
## ngrok-open       : Ouvre le dashboard ngrok
ngrok-open:
...
docker-clean:
	@read -p "Confirmer suppression Docker GNL [oui/NON] : " c && [ "$$c" = "oui" ] || exit 1
...
## docker-ps        : Conteneurs GNL
docker-ps:
	@docker ps --filter "name=gnl_" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"

## docker-logs      : Logs Docker GNL
docker-logs:
	@docker logs --tail=30 gnl_edge_node 2>/dev/null || true
	@docker logs --tail=20 gnl_gemma4    2>/dev/null || true

## backup           : Sauvegarde config + code
backup:
	@mkdir -p $(PROJECT_DIR)/backups
...
.env:
	...
	@printf '%s\n' \
	  ...
	  'GEMMA4_TEMPERATURE=0.15' \
	  '' \
	  '# IA — Gemma4 uniquement (pas dAnthropic)' \
	  'GNL_AI_PROVIDER=gemma4' \
	  'GNL_AI_INTERVAL=30' \
...
help:
	...
	@grep -E '^## ' $(MAKEFILE_LIST) | sed 's/## //' | \
	  awk 'BEGIN{FS=":"} \
	    /^[A-Z]/ {printf "  $(BOLD)$(YELLOW)%-22s$(NC) %s\n", $$1, $$2; next} \
	...
	@echo -e "  $(GREEN)make start$(NC)             → Lance tout (télécharge Gemma4 si absent, détecte Codespaces/RPi)"
	@echo -e "  $(GREEN)make download-gemma4$(NC)   → Télécharge le modèle Gemma4 (~3.5 GB)"
	@echo -e "  $(GREEN)make gemma4-status$(NC)     → Vérifie que Gemma4 est opérationnel"
